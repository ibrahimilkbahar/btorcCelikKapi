...
JOIN t_product_case_type prct on off.productCaseTypeId = prct.ID
...
SELECT orr.Id orderId,	
    ...
    CONCAT(CONCAT(CONCAT(CONCAT(`di`.`ProductionWidth`, ' / '), `di`.`ProductionHeight`), ' / '), `di`.`ProductionDepth`) AS `Dimension`,
...
FROM
    ...
    JOIN t_dimension di ON oo.dimensionId = di.ID
